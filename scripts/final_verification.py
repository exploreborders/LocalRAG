    ...
    try:
        ...
        if chapter_count > 0:
            ...
            for chapter in chapters:
                print(f"  '{chapter.chapter_title[:30]}...' ({chapter.word_count} words, {chapter.section_type}, level {chapter.level})")
        ...
        path_chunks = db.execute(text("SELECT COUNT(*) FROM document_chunks WHERE chapter_path IS NOT NULL")).scalar()
        type_chunks = db.execute(text("SELECT COUNT(*) FROM document_chunks WHERE section_type IS NOT NULL")).scalar()
        relevance_chunks = db.execute(text("SELECT COUNT(*) FROM document_chunks WHERE content_relevance IS NOT NULL")).scalar()

        print(f"  Total chunks: {total_chunks}")
        print(f"  With chapter_title: {chapter_chunks} ({chapter_chunks/total_chunks*100:.1f}%)")
        ...
        print(f"  With content_relevance: {relevance_chunks} ({relevance_chunks/total_chunks*100:.1f}%)")
        ...
        if chapter_chunks > 0:
            ...
            for chunk in sample_chunks:
                print(f"  Chunk {chunk.chunk_index}: '{chunk.chapter_title[:25]}...' ({chunk.section_type}, rel: {chunk.content_relevance:.2f})")

        # Overall assessment
        print("\n🎯 OVERALL ASSESSMENT:")

        all_populated = (
            docs[0].content_len > 0 and
            docs[0].chapter_status == 'populated' and
            docs[0].toc_status == 'populated' and
            docs[0].structure_status == 'populated' and
            chapter_count > 0 and
            chapter_chunks == total_chunks and
            path_chunks == total_chunks and
            type_chunks == total_chunks and
            relevance_chunks == total_chunks
        )
