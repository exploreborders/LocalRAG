    ...
    def _extract_docling_structure(self, docling_document) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Fallback structure extraction also failed: {fallback_e}"
                )
        ...
        for i, line in enumerate(lines):
            ...
            if len(line) < 200:  # Reasonable header length
                ...
                if line.startswith("# "):
                    ...
                    logger.debug(
                        f"  -> Matched markdown header level 1 (level {level})"
                    )
                elif line.startswith("## "):
                    is_header = True
                    level = 2  # Section level
                    logger.debug(
                        f"  -> Matched markdown header level 2 (level {level})"
                    )
                elif line.startswith("### "):
                    is_header = True
                    level = 3  # Subsection level
                    logger.debug(
                        f"  -> Matched markdown header level 3 (level {level})"
                    )
