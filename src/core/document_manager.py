...
from database.models import Document, DocumentChunk, DocumentChapter, DocumentEmbedding, DocumentTag, DocumentCategory, DocumentTagAssignment, DocumentCategoryAssignment, SessionLocal
...
class TagManager:
    ...
    def remove_tag_from_document(self, document_id: int, tag_id: int) -> bool:
        ...
        assignment = self.db.query(DocumentTagAssignment).filter(
            DocumentTagAssignment.document_id == document_id,
            DocumentTagAssignment.tag_id == tag_id
        ).first()
    ...
    def get_document_tags(self, document_id: int) -> List[DocumentTag]:
        ...
        assignments = self.db.query(DocumentTagAssignment).filter(
            DocumentTagAssignment.document_id == document_id
        ).all()
    ...
    def get_popular_tags(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        popular_tags = self.db.query(
            ...
            DocumentTag.color,
            func.count(DocumentTagAssignment.document_id).label('document_count')
        ).outerjoin(DocumentTagAssignment).group_by(DocumentTag.id).order_by(
            desc(func.count(DocumentTagAssignment.document_id))
        ).limit(limit).all()
    ...
    def get_related_tags(self, tag_name: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            doc_ids = [assignment.document_id for assignment in
            ...
            ).group_by(DocumentTag.id, DocumentTag.name, DocumentTag.color).order_by(
                func.count(DocumentTagAssignment.tag_id).desc()
            ).limit(limit).all()
            ...
            return [
                {
                    'name': rt.name,
                    'color': rt.color,
                    'co_occurrence': rt.co_occurrence
                }
...
class CategoryManager:
    ...
    def remove_category_from_document(self, document_id: int, category_id: int) -> bool:
        ...
        assignment = self.db.query(DocumentCategoryAssignment).filter(
            DocumentCategoryAssignment.document_id == document_id,
            DocumentCategoryAssignment.category_id == category_id
        ).first()
    ...
    def get_document_categories(self, document_id: int) -> List[DocumentCategory]:
        ...
        assignments = self.db.query(DocumentCategoryAssignment).filter(
            DocumentCategoryAssignment.document_id == document_id
        ).all()
    ...
    def get_category_hierarchy(self, category_id: int) -> List[DocumentCategory]:
        ...
        while current:
            ...
            if current.parent_category_id:
                current = self.db.query(DocumentCategory).filter(
                    DocumentCategory.id == current.parent_category_id
                ).first()
            else:
                current = None
...
class DocumentProcessor:
    ...
    def process_document(self, file_path: str, filename: str = None, progress_callback: Callable = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self._index_document(document, chunks, embeddings)
    ...
    def _index_document(self, document: Document, chunks: List[Dict[str, Any]], embeddings: List[List[float]]):
...
class UploadProcessor:
    ...
    def process_files(self, file_paths: List[str], progress_callback: Callable = None) -> Dict[str, Any]:
        """
    ...
    def process_single_file(self, file_path: str, filename: str = None, file_hash: str = None, force_enrichment: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if existing_doc and force_enrichment:
                result = self.reprocess_existing_document(existing_doc, result, file_path)
            elif result.get('success'):
                # Store the document in database if it's a new upload
                result = self._store_processed_document(result, file_path, filename or Path(file_path).name)
    ...
    def reprocess_existing_document(self, existing_doc: Document, processing_result: Dict[str, Any], file_path: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.db.query(DocumentChapter).filter(DocumentChapter.document_id == existing_doc.id).delete()
            ...
            if chunks_created > 0:
                chunks = self.db.query(DocumentChunk).filter(DocumentChunk.document_id == existing_doc.id).all()
                chunk_texts = [chunk.content for chunk in chunks]
                embeddings = create_embeddings(chunk_texts)

                # Clear existing embeddings
                self.db.query(DocumentEmbedding).filter(DocumentEmbedding.document_id == existing_doc.id).delete()
                ...
                # Update search index
                processor = DocumentProcessor()
                processor._index_document(existing_doc, [{'content': chunk.content, 'id': chunk.id} for chunk in chunks], embeddings)
    ...
    def _store_processed_document(self, processing_result: Dict[str, Any], file_path: str, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if chunks_created > 0:
                ...
                # Index in search
                processor = DocumentProcessor()
                processor._index_document(doc, [{'content': chunk.content, 'id': chunk.id} for chunk in chunks], embeddings)
