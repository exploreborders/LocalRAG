...
class DocumentChapter(Base):
    ...
    section_type: Mapped[str] = mapped_column(
        String(50), default="chapter"
    )  # 'chapter', 'section', 'subsection'
    parent_chapter_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("document_chapters.id")
    )
...
class DocumentChunk(Base):
    ...
    chapter_title: Mapped[Optional[str]] = mapped_column(
        String(255)
    )  # Chapter/section this chunk belongs to
    chapter_path: Mapped[Optional[str]] = mapped_column(
        String(500)
    )  # Hierarchical path (e.g., "1.2.3")
    section_type: Mapped[Optional[str]] = mapped_column(
        String(50)
    )  # 'chapter', 'section', 'subsection', 'paragraph'
    content_relevance: Mapped[Optional[float]] = mapped_column(
        Float
    )  # 0-1 score for content density

    # Topic classification
    primary_topic: Mapped[Optional[str]] = mapped_column(
        String(100)
    )  # Main topic this chunk relates to
    topic_relevance: Mapped[Optional[float]] = mapped_column(
        Float
    )  # 0-1 relevance score to primary topic
...
class DocumentCategory(Base):
    ...
    parent_category_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("document_categories.id")
    )
