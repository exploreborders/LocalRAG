...
class KnowledgeGraph:
    ...
    def build_relationships_from_cooccurrence(self, min_occurrences: int = 3) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        tag_assignments = self.db.query(
            DocumentTagAssignment.document_id,
            DocumentTag.name.label('tag_name')
        ).join(DocumentTag).all()
    ...
    def get_related_tags(self, tag_names: List[str], depth: int = 2,
    ...
    def get_related_categories(self, category_names: List[str], depth: int = 1) -> Set[str]:
        ...
        while queue:
            ...
            parent = self.db.query(DocumentCategory).filter(
                DocumentCategory.name == current_cat
            ).first()

            if parent and parent.parent_category_id:
                parent_cat = self.db.query(DocumentCategory).filter(
                    DocumentCategory.id == parent.parent_category_id
                ).first()
    ...
    def expand_query_context(self, tag_names: List[str], category_names: List[str],
        ...
        return {
            ...
            'total_expansions': len(expanded_tags) + len(expanded_categories) + len(tag_related_categories)
        }

    def find_documents_by_relationships(self, tag_names: List[str],
        ...
        if tag_names and category_names:
            ...
            base_query = self.db.query(Document).select_from(Document)\
                .join(DocumentTagAssignment, Document.id == DocumentTagAssignment.document_id)\
                .join(DocumentCategoryAssignment, Document.id == DocumentCategoryAssignment.document_id)\
        ...
        for doc in direct_docs:
            ...
            doc_categories = [assignment.category.name for assignment in doc.categories if assignment.category]
        ...
        if include_related:
            ...
            related_categories = self.get_related_categories(category_names) if category_names else set()
            ...
            if related_tags or related_categories:
                ...
                if related_tags:
                    from sqlalchemy import select
                    related_tag_ids = select(DocumentTag.id).where(DocumentTag.name.in_(related_tags)).scalar_subquery()
                ...
                if related_categories:
                    from sqlalchemy import select
                    related_cat_ids = select(DocumentCategory.id).where(DocumentCategory.name.in_(related_categories)).scalar_subquery()
    ...
    def get_graph_statistics(self) -> Dict[str, Any]:
        ...
        return {
            'tags': {
                'total': tag_count,
                'with_relationships': tag_relationships
            },
            'categories': {
